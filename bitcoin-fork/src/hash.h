...
/** CS-Coin: 256-bit hash using SHA-3 (Keccak-256) instead of double SHA-256. */
class CHash256 {
private:
    SHA3_256 sha3;
public:
...
/** CS-Coin: writer stream that computes a 256-bit hash using SHA-3. */
class HashWriter
{
private:
    SHA3_256 ctx;
...
public:
    ...
    {
        ctx.Write(std::span<const unsigned char>(UCharCast(src.data()), src.size()));
    }

    /** Compute the SHA-3 hash of all data written to this object.
     *
     * Invalidates this object.
     */
    uint256 GetHash() {
        uint256 result;
        ctx.Finalize(std::span<unsigned char>(result.begin(), SHA3_256::OUTPUT_SIZE));
        return result;
    }
